...
export type OpenAICompatibleModel = 'mini' | 'standard' | 'codex' | 'frontier' | 'local' | 'grok';
...
export interface OpenAICompatibleProviderProfile {
  ...
  modelDefaults: Partial<Record<OpenAICompatibleModel, string>>;
...
export class OpenAICompatibleExecutor {
  ...
  private profile: OpenAICompatibleProviderProfile;
  ...
  constructor(
    ...
    const key = apiKey || process.env[profile.apiKeyEnv] || profile.defaultApiKey;

    if (!key) {
      throw new Error(
        profile.missingApiKeyMessage ||
        `${profile.apiKeyEnv} environment variable is required for ${profile.providerName} provider`
      );
    }

    this.client = new OpenAI({
      apiKey: key,
      baseURL: process.env[profile.baseUrlEnv] || profile.defaultBaseUrl
    });

    this.defaultModel =
      model ||
      parseOpenAICompatibleModel(process.env[`${profile.modelEnvPrefix}_MODEL`]) ||
  ...
  async execute(task: AgentTask): Promise<AgentResult> {
    ...
    const model = resolveOpenAICompatibleModelId(tier, this.profile);
    ...
    try {
      ...
      return {
        ...
        error: sanitizeErrorMessage(
          `${this.profile.providerName} execution error for task ${taskId}: ${error?.message || String(error)}`
        ),
        model
      };
    }
  ...
  private buildSystemPrompt(taskType: string): string {
    return [
      `You are Sandyaa, an autonomous security code-audit agent running through the ${this.profile.providerName} provider.`,
...
export function resolveOpenAICompatibleModelId(
  ...
  const envKey = `${profile.modelEnvPrefix}_${model.toUpperCase()}_MODEL`;

  return (
    process.env[envKey] ||
    profile.modelDefaults[model] ||
    profile.modelDefaults[profile.defaultModel] ||
    String(model)
  );
}
...
function parseJSONResponse(text: string): any {
  ...
  try {
    ...
    if (!extracted) {
      throw new Error(`OpenAI-compatible response did not contain parseable JSON: ${cleaned.slice(0, 500)}`);
    }
...
function parseOpenAICompatibleModel(value?: string): OpenAICompatibleModel | undefined {
  if (
    value === 'mini' ||
    value === 'standard' ||
    value === 'codex' ||
